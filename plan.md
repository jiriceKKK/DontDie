...
> Completion record
> - Status: Complete
> - Completed at: 2026-09-02T20:32:17+02:00
> - Files materially changed: `js/data/types.js`, `js/data/indexedDb.js`, `js/data/repository.js`, `js/data/documentRepository.js`, `js/data/habitRepository.js`, `js/data/outbox.js`, `js/data/reconcile.js`, `js/data/conflicts.js`, `js/data/migrations.js`, `js/ui/syncStatus.js`, `js/ui/saveFeedback.js`, `js/db.js`, `js/main.js`, `js/state.js`, `js/sync.js`, `js/habitConfig.js`, `js/habitStimLink.js`, `js/split/store.js`, `js/mental/store.js`, `js/mental/texts/store.js`, `js/stimulation/store.js`, `js/school/store.js`, `js/tabs/today.js`, `js/tabs/settings.js`, `js/tabs/split.js`, `js/navigation.js`, `js/modes/controller.js`, `index.html`, `style.css`, `sw.js`, `README.md`, `docs/security-deployment.md`, `supabase/migrations/002_revision_and_archive_support.sql`, `supabase/verify/002_revision_and_archive_support.sql`, `scripts/notify-status.mjs`, `scripts/secret-scan.mjs`, `scripts/db-verify.mjs`, `tsconfig.core.json`, `package.json`, `package-lock.json`, `eslint.config.js`, `playwright.config.js`, `.env.example`, `tests/unit/data/*` (4 files), `tests/unit/notifyStatus.test.mjs`, `tests/e2e/offline-sync.spec.js`, `tests/e2e/conflict-recovery.spec.js`, `tests/e2e/split-motion.spec.js`, `tests/e2e/swipe-performance.spec.js`, `tests/e2e/navigation-controls.spec.js`, `tests/e2e/fixtures/fakeSupabase.js`, `tests/e2e/fixtures/seed.js`, `tests/e2e/auth.spec.js`, `tests/e2e/rendering.spec.js`.
> - Architecture: one IndexedDB database `dontdie_local_v2` with `documents`, `habitLogs`, `customHabits`, `outbox`, `conflicts` and `meta` stores. `js/main.js` authenticates, hydrates local state, builds the shell, and only then reconciles in the background — no cloud read blocks tab construction. Every module action delegates to `js/data/repository.js`; `js/state.js` is now documented and used as an in-memory read cache rather than the persistence source of truth. A mutation writes the record AND its outbox operation in one transaction, so a returned success genuinely means "durably stored, and the cloud write is guaranteed to be attempted". `js/data/reconcile.js` is the only place a remote write happens and drains the queue sequentially, which is what makes one-write-in-flight-per-document and late-response protection structural rather than incidental. Collapse is deliberately conservative: only `save`/`upsert` pairs with the same `(kind, entity)` merge, and any `create`/`delete`/`deleteAll` is a boundary that is never collapsed across.
> - Database: `supabase/migrations/002_revision_and_archive_support.sql` adds `revision bigint NOT NULL` plus a `BEFORE INSERT OR UPDATE` trigger that maintains `revision` and `updated_at` server-side, so client-supplied values cannot defeat compare-and-set. Rehearsed with `npm run db:verify` against a disposable Postgres 17 container (001 + verify 001 + 002 + verify 002, sections A–E all PASS), proven idempotent by a second run, and proven backward compatible by replaying the deployed Phase 1 client's plain `upsert` and confirming the server still stamped both columns. A fresh production backup checkpoint was taken with `pg_dump --format=custom` BEFORE applying, stored outside version control, and verified by restoring it into a disposable database: all seven tables and all 164 rows came back. The migration was then applied to production over SSL with `ON_ERROR_STOP=1` — 5 installed document tables, `habit_config` skipped as designed. Production verification 002 A–E all PASS; verification 001 A–F re-run afterwards and all PASS, so the authorization boundary is intact. Row counts unchanged: `habit_logs` 152, `custom_habits` 7, one owned row in each of five JSON stores.
> - Split first-tap defect — diagnosed, not assumed: a Playwright probe sampled `.split-day-body` height across one activation on the deployed code. Before the click: `h=0`. Synchronously after: `h=115`, inline `115px`, `.open` already applied. At +2 frames, +60 ms, +140 ms: `115` throughout; at +440 ms the inline height was simply cleared. Root cause: `wireContent` toggled `.open` **before** `animateCollapse` measured the start height, and the open resting style is `height: auto`, so the measurement returned the fully-open layout. Start and end were identical, no transition ran, `transitionend` never fired, and the 360 ms fallback only stripped the inline height. The close path measured `0 → 0` and was equally dead. The "double tap" was the accidental case: a second activation inside that 360 ms window still found the stale inline pixel height, so `115 → 0` did animate (measured at 103 px mid-flight). Fix: measure the genuinely current height first, then apply the state, then measure the natural end height once, lock the start, and start the transition on the next frame; runs are owned by a token so a stale timer or `transitionend` can never clobber a reversal. The headers are now real `<button>`s with `aria-expanded`/`aria-controls`, so keyboard activation goes through the identical path.
> - Swipe defect — measured before and after. Before: a document-wide non-passive `touchmove` wrote `transform` once per event with no frame coalescing (28 writes across 28 events); finishing a swipe rendered the destination panel synchronously in the settle frame (up to 2.8 ms per tab on an unthrottled desktop CPU); a mode change rendered all five panels at once (30 ms for Physical); a 97 ms long task was observed; and `will-change: transform` was pinned permanently. After: the gesture is slider-scoped Pointer Events with pointer capture and `touch-action: pan-y`; `pointermove` does arithmetic only — geometry is read once at gesture start; transform writes are coalesced to one per animation frame; the destination render is deferred two frames past the settle; and `will-change` is applied per gesture and cleared. Measured over ten consecutive swipes on populated fixture data with CPU throttling, at 390 × 844 and 320 × 700: 120 movement writes over ~460 frames, **max 1 transform write per movement frame**, **0 database calls during pointer movement**, **0 long tasks over 50 ms** (none at all), `will-change` cleared, and no duplicate listener after fifteen mode changes.
> - Navigation control: `#mode-switcher` and its label, chevron and CSS rules are gone from the markup, the controller and the stylesheet; the controller no longer looks the element up, so there is no missing-element error and no dead listener. `#mode-fab` is the single mode picker and Home shortcut, now with `aria-haspopup`, a tracked `aria-expanded`, a current-mode accessible name, and keyboard parity (Enter/Space opens the picker, Shift+Enter is the accessible equivalent of holding for Home). On mobile the header is hidden outright rather than left as an empty translucent bar, and panel padding was increased so the lower control cannot cover the last actionable item. Asserted at 320 × 700, 390 × 844 and 844 × 390 landscape, with a shrunk-viewport keyboard case, that no actionable control is overlapped and the page never scrolls horizontally.
> - Validation: `npm run validate` — pass, exit 0: `check-syntax` 114/114 via `node --check`; `eslint .` 0 errors and 21 budgeted legacy warnings (unchanged cap); `secret-scan` clean over 138 files with 10 audited allowances, now including a structural rule that fails the build if anything under `js/`, `tests/e2e/`, `vendor/`, `index.html` or `sw.js` so much as names a notifier or the webhook variable; `npm run typecheck:core` clean; `node --test` 104/104 unit; Playwright 116/116 across `mobile-390x844`, `desktop-1280x900` and a dependent `gesture-performance` project that runs uncontended. Ten consecutive full-suite runs were green before this record was written. Scenario coverage: local render before the cloud answers, localStorage→IndexedDB migration, interrupted-migration recovery, offline mutation plus offline reload, durable outbox survival, reconnect convergence (exactly one row, not one per retry), five offline toggles collapsing to one final-state write, same-device write ordering, stale-revision conflicts with both snapshots retained and downloadable, "keep this device" / "use cloud" resolution, parked operations that never touch the cloud, tombstones for logs and custom habits surviving a racing hydrate, delete during an in-flight write, owner-bound queued operations, no-IndexedDB and no-cloud degradation, the missing `habit_config` table, linked habit/stimulation convergence, Split first-tap and reversal and reduced motion and keyboard, swipe traces and video at two mobile viewports, retained lower mode control, and the status notifier's refusals.
> - Deviations: (1) Archive/tombstone semantics for activities, tests and custom habits are deliberately **not** in migration 002. Phase 3 specifies the archive model and its read-path rules; adding a half-defined `deleted_at` now would change what the deployed client sees before anything filters on it. 002 delivers the revision/compare-and-set support Phase 2 actually requires, and tombstones live on the device. (2) Where the browser refuses a local database (private mode, blocked upgrade), the app writes straight to the cloud and reports the true outcome rather than becoming read-only — and if neither is available it says the change could not be saved instead of showing a success it cannot back up. (3) A real `env(safe-area-inset-*)` cannot be emulated by the runner, and injecting a stylesheet is correctly refused by the page's own CSP, so that check asserts the layout rule that produces the behaviour plus the zero-inset geometry, and says so in the test. (4) Two development dependencies were added: `typescript` for the core typecheck the plan requires, and `fake-indexeddb` for the deterministic data-layer unit tests. Neither ships to the browser. (5) The gesture-performance measurements run in their own Playwright project that depends on the two viewport projects, so they execute last and uncontended; a long-task budget measured against a saturated runner is noise, not evidence. (6) Legacy `localStorage` keys are still written as well as read — the plan requires retaining them for one recovery cycle, and they are also the read path when IndexedDB is unavailable.
> - Unresolved issues: P2 — the module renderers still emit inline `style` attributes and still mutate `state` directly for transient view state; persistence and mutation now go through repositories, but the fuller renderer/action separation belongs with the design-system and workflow phases. P2 — `habit_config` does not exist in production, so habit configuration is device-local plus its legacy mirror until a later phase decides whether to install the table. P2 — the pre-Phase-1 PIN and its hash remain in published Git history and must be treated as public. P2 — the Discord webhook was supplied over a chat channel; rotate it if that transcript is not tightly controlled (`docs/security-deployment.md` §6).
> - Discord notification: Sent at 2026-09-02T20:32:17+02:00 via `scripts/notify-phase.mjs` (HTTP 204, endpoint read from `DONTDIE_DISCORD_WEBHOOK_URL` only).
